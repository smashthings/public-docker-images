...
docker buildx build -t smasherofallthings/$IMAGE_NAME:arm7-latest -t "smasherofallthings/$IMAGE_NAME:arm7-$(date '+%Y-%m-%d')-b${DRONE_BUILD_NUMBER}" -f $IMAGE_NAME/Dockerfile --platform linux/arm/v7 --output type=docker $IMAGE_NAME
